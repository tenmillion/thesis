...
% ４人の場合
\ecmembers{Kazushi Ikeda, Professor}{(Supervisor)}
          {Shoji Kasahara, Professor}{(Co-supervisor)}
          {Takatomi Kubo, Assistant Professor}{(Co-supervisor)}
          {Mitsuo Kawato, Professor}{(Co-supervisor)}
\eaddcmembers{Toshitaka Yamakawa, Assistant Professor (Shizuoka U.)}{(Member)}{}{}{}{}{}{}
%
%
%
% キーワード５～６個 (in LaTeX)
...
where $g_{syn}^{i,j}$ is the conductance of the postsynaptic ion channel, and $E_{syn}$ is the reversal potential of the ion species used by the channel. In response to the arrival of an action potential at the presynaptic compartment at time $t_{AP}$, $g_{syn}^{i,j}$ changed according to the alpha function
$$g_{syn}^{i,j}(t)=\tau_A(t-t_{AP}-D^{i,j}) \exp(1-\tau_B(t-t_{AP}-D^{i,j})),$$
where $\tau_A$ and $\tau_B$ are the opening and closing time constants for the postsynaptic ion channel, specified for each type of synapse, and $D^{i,j}$ is the synaptic delay for synapse ($i,j$). This delay was assigned randomly to each synapse according to a normal distribution, and remained fixed for all simulations. The temperature dependence of the synaptic currents was not modeled.
...
\subsection{Methods}
\label{methods2}
\subsubsection{Overview of Simulation 2}
A simplified model of mammalian cortex was constructed, based on Wang and Buzs{\'a}ki's model of mammalian hippocampus\cite{Wang1996}. As this model was an abstract model omitting details specific to hippocampal cortex such as the structure of connections, and fast-spiking interneurons, the type of neuron on which their model was based, are also abundant in neocortex, we reasoned that it can be used to model neocortex without modification.

Similar to Simulation 1, neurons with Hodgkin-Huxley-type membrane conductances were coupled to one another through conductance-based synapses, and their activity was simulated for 1300 msec.

Simulation 2 differed from Simulation 1 on five important points. First, the neuron model (i.e. the parameters of the Hodgkin-Huxley-type equations) was changed from the Borg-Graham model\cite{Borg-Graham1987} to the Wang-Buzs{\'a}ki model\cite{Wang1996}. %Although the Borg-Graham model was also confirmed to have Class I excitability\ref{BorgGrahamClassI}(*todo),
The Wang-Buzs{\'a}ki model was chosen because its dynamical properties had already been extensively analyzed by Sato\cite{Sato2013}.

Second, the number of compartments per neuron was reduced to 1, and a spherical shape with surface area of 400 um2 (*check) was assumed for the compartment. Additional compartments can alter the phase-response characteristics of a neuron[(*to look up)], and to facilitate comparison between Sato's analysis of single cells and the results of our network simulation, this potential interference was removed.

Third, the parameters of the synaptic connections were made to be identical across all synapses. Again, this was to facilitate analysis by excluding factors that explicitly introduced heterogeneity into the model.

Fourth, extrinsic input to the network was modeled by a constant current of identical amplitude for all neurons, as opposed to the transient Poisson input used in Simulation 1. With persistent input, it is easier for the weakly connected network to sustain firing without synchronous firing.

Finally, the number of connections per neuron was randomized, as opposed to the fixed numbers of connections used in Simulation 1. The assumption that all neurons in an actual cortical network have the same number of connections is unrealistic. Furthermore, Wang and Buzs{\'a}ki showed that a network with a constant number of connections per neuron for all neurons is much more susceptible to complete synchrony, even when the number itself is small compared to the number of targets available. In addition to making the model more realistic, we wanted to avoid this potentially synchronizing factor.


\subsubsection{Modeling of Epileptic Seizures}
In the inhibitory-only network, no attempt was made to explicitly model epileptic activity, in order to keep the model applicable to a wide range of etiologies of epilepsy. 

In the mixed network, we studied the effect of varying the ratio between inhibitory and excitatory synaptic weights. One possible cause for epilepsy is the reduction of the efficacy of inhibitory synapses. 
